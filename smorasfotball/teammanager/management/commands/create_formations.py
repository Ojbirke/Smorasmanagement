...
class Command(BaseCommand):
    help = 'Create formation templates for different player counts'
    ...
    def handle(self, *args, **options):
        ...
        for formation in formations:
            ...
            if FormationTemplate.objects.filter(name=formation["name"]).exists():
                self.stdout.write(self.style.WARNING(f"Formation {formation['name']} already exists, skipping."))
                continue
            ...
            self.stdout.write(self.style.SUCCESS(f"Created formation: {template}"))
            created_count += 1
        
        if created_count > 0:
            self.stdout.write(self.style.SUCCESS(f"Created {created_count} new formation templates."))
        else:
            self.stdout.write(self.style.WARNING("No new formation templates were created. All already exist."))